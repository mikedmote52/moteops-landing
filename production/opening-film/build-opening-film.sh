...
chaos_end="5.5"
interface_hold="1.8"
cleanup_end="7.966667"
master_duration="28.0"

bash "$production/capture-plates.sh"
bash "$production/capture-laptop-plates.sh"
bash "$production/capture-monitor-plates.sh"
bash "$production/build-laptop-discovery.sh"
...
ffmpeg -hide_banner -loglevel warning -y \
  ...
  -i "$rendered/laptop-discovery.mp4" \
  ...
  -framerate 24 -loop 1 -t "$interface_hold" -i "$rendered/monitor-approval-queue.png" \
  ...
  -filter_complex "
    [0:v]${normalize},trim=start=0:end=${chaos_end},setpts=PTS-STARTPTS[v0];
    [1:v]${normalize},trim=duration=5.0,setpts=PTS-STARTPTS[v1];
    [2:v]${normalize},trim=duration=${interface_hold},setpts=PTS-STARTPTS[v2];
    ...
    [6:v]${normalize},trim=start=5.8:end=${cleanup_end},setpts=PTS-STARTPTS[v6];
    [7:v]${normalize},trim=start=0:end=8.0,setpts=PTS-STARTPTS[v7];
    [v0][v1][v2][v3][v4][v5][v6][v7]concat=n=8:v=1:a=0,settb=1/24[story];
    ...
    [o3][pressure4]overlay=enable='between(t,3.5,5.45)'[o4];
    [o4][pressure5]overlay=enable='between(t,4.1,5.45)'[o5];
    [o5][beach0]overlay=enable='between(t,24.1,28.0)'[o6];
    [o6][beach1]overlay=enable='between(t,25.9,28.0)'[master]
  " \
...
node "$production/update-opening-manifest.mjs"
